...
bool capacity_frps_session_probe(void)
{
    ...
    for (unsigned waited_ms = 0; waited_ms < 10000 && !trusted_time(NULL); waited_ms += 20)
        vTaskDelay(pdMS_TO_TICKS(20));
    if (!trusted_time(NULL)) {
        report_status("clock_invalid", EFRP_TIME_UNTRUSTED, NULL);
        return false;
    }
    ESP_LOGI(TAG, "base_sntp_ready=1 unix_time=%lld", (long long)time(NULL));
