...
上游修正原型只调整三处：无 WASI 时不编译文件适配、明确包含 `<sys/stat.h>`、在 IDF 无可执行堆能力时拒绝执行映射并保留普通映射。实验保留 `CONFIG_ESP_SYSTEM_MEMPROT=y`，未定义虚假的 `MALLOC_CAP_EXEC`，也未修改固定 SDK 或 `managed_components`。本仓正式依赖仍指向未修正的 WAMR SHA；上游源码修复并取得可消费的精确提交前，P6-02 的 C3 构建条件未满足。
