...
这是**成功结果的期限裁决**，不是可抢占的墙钟超时。WAMR 与 ESP-IDF 同步原生调用在执行期间不能由这个检查中断；没有原生导入的 guest 只会在返回后受到时间裁决，实际返回耗时仍取决于指令预算与调度。当前四个导入均为有限、无等待操作，因此此切片改善了当前入口的超期成功误报和后续导入副作用边界；未来若加入网络、Flash、等待或其他可能阻塞的 SDK 调用，必须另行设计可取消的异步合同，不能把这里的事后检查当作单次调用返回的硬上界。签名 manifest 的 `host_call_timeout_ms` 目前仅用于包槽独立限额准入，尚无公开安装 API 将其绑定到私有运行限制；不得据此宣称签名期限已在设备运行期执行。
