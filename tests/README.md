...
Python 测试使用每次生成的 RSA-3072 临时测试密钥，覆盖确定性归档、签名/错误公钥与错误 PSS 参数、清单顶层及嵌套重复键、未知字段和数值边界、Wasm start/导入白名单/构造入口、导入能力与 manifest 的对应、成员路径/顺序/类型/长度白名单、校验和、成员填充、额外成员、尾随非零或全零块及容量拒绝。另有固定占位签名的 host 编码回归向量，摘要见[包工具](../tools/README.md)；占位签名不能用于验签或发布。C 测试覆盖组件扫描的正常、截断、start、导入与隐式构造入口。测试密钥只存在系统临时目录，不进入 Git。
